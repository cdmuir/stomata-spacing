...
Stomatal density of *Arabidopsis thaliana* the `r nrow(single_surface_results)` leaf surfaces measured ranged from 12 to 93 (units) with high light leaves ranging from 93 to 55 (units), medium light from 15 to 35 (units), and low light from 12 to 42 (units). Leaves were amphistomatous with a mean stomatal ratio of 0.45.
...
<!-- WORKING HERE -->
Many leaf surface (`r nni[['n1']]` of `r nni[['n1']] + nni[['n2']]`, `r round(100 * nni[['n1']] / (nni[['n1']] + nni[['n2']]), 1)`%) are significantly overdispersed compared to a random uniform distribution, but none were close to an ideal hexagonal pattern (dispersion index = 1; \autoref{fig:single-surface}). Dispersion index differs significantly among light treatments (ANOVA, $F = `r$) is significantly greater Additionally, we hypothesized that as CO$_2$ is more limiting to photosynthesis under high light, the dispersion index would closer to 1 in high light...WORKING HERE would be more uniformly dispersed in plants grown in high light than plants grown in low and medium light. The data also fail to support this hypothesis as there is no strong, discernible trend between light and stomatal patterning. Interestingly, adaxial leaf surfaces were more uniformly dispersed than associated abaxial leaf surfaces across all light treatments (F$_\text{1,126}$ = 28.8; p-value < 0.001). Rather than regulate stomatal patterning in response to light regimes, plants respond by increasing stomatal density (\autoref{fig:density}). Stomatal density drastically increased in plants grown under high light, validating the long held hypothesis that light strongly influences stomatal density (F$_\text{2,126}$ = 680.7; p-value < 0.001).
