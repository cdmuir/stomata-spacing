---
title: Does stomatal patterning in amphistomatous leaves minimize the CO$_2$ diffusion path length with leaves of *Arabidopsis thaliana*?
date: "`r Sys.Date()`"
author:
  - name: Jacob L. Watts
...
abstract:
  - "Photosynthesis is co-limited by multiple factors depending on the plant and its environment. These include biochemical rate limitations, internal and external water potentials, temperature, irradiance, and carbon dioxide (CO2). Amphistomatous leaves have stomata on both abaxial and adaxial leaf surfaces. This feature is considered an adaptation to alleviate CO2 diffusion limitations in productive environments where other factors are not limiting as the diffusion path length from stomate to chloroplast is effectively halved. Plants can also reduce CO2 limitations through other aspects of optimal stomatal anatomy: stomatal density, distribution and patterning, and size. A number of studies have demonstrated that stomata are overdispersed on a single leaf surface; however, much less is known about stomatal anatomy in amphistomatous plants, especially the coordination between leaf surfaces, despite their prevelance in nature and near ubiquity among crop species. Here we use novel spatial statistics based on simulations and photosynthesis modelling to test hypotheses about how amphistomatous plants may optimize CO2 limitations in the model angiosperm Arabidopsis thaliana grown in different light environments. We find that 1) stomata are overdispersed, but not ideally dispersed, on both leaf surfaces across all light treatments; 2) abaxial and adaxial leaf surfaces are independent; and 3) the theoretical improvements to photosynthesis from abaxial-adaxial stomatal coordination are miniscule (~1%) across the range of feasible traitspace. However, we also find that 4) stomatal size is correlated with the mesophyll volume that it supplies with CO2, suggesting that plants may optimize CO2 diffusion limitations through alternative pathways other than ideal, uniform stomatal spacing. We discuss the myriad developmental, physical, and evolutionary constraits which may prohibit plants from reaching the theoretical adaptive peak of uniform stomatal spacing and inter-surface stomatal coordination. These findings contribute to our understanding of amphistomatous plants' exploration of the leaf anatomy adaptive landscape."
keywords:
  - amphistomy
...
## Stomatal distribution is non-random, but far from ideal
