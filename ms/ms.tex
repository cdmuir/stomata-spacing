...
% Create labels for Addresses if the are given in Elsevier format
   \MyNewLabel{ADDRTXT}{Colgate University}{%
  13 Oak Drive, Hamilton, NY 13346%
 }
   \MyNewLabel{ADDRTXT}{University of Hawaii at Manoa}{%
...
\title[]{Spatial Coordination Of Stomatal Patterning Between Leaf
Surfaces In Amphistomatous \emph{Arabidopsis thaliana} Incurs No
...
\author[%
\addrlblbycode{Colgate University}%
,\refstepcounter{correspcnt}\setcounter{thisauthcorresp}{\value{correspcnt}}\fnsymbol{thisauthcorresp}%
%
%
]{Jacob L. Watts}

\addrbycode{Colgate University}

\corresp[\fnsymbol{thisauthcorresp}]{Corresponding author. \href{mailto:jwatts@colgate.edu}{\nolinkurl{jwatts@colgate.edu}}}
...
\author[%
\addrlblbycode{University of California Davis}%
%
%
,\corresplblbycode{1}%
]{Thomas N. Buckley}

\addrbycode{University of California Davis}
...
\author[%
\addrlblbycode{University of Hawaii at Manoa}%
%
%
,\corresplblbycode{1}%
]{Christopher D. Muir}

\addrbycode{University of Hawaii at Manoa}
...
(\(gs_\text{s,op}\)) rather than maximum conductance
(\(gs_\text{s,max}\)) such that stomatal apertures are most responsive
...
determine a leaf's theoretical \(gs_\text{s,max}\)
\citep{sack_developmental_2016}, which is proportional to
\(gs_\text{s,op}\) \citep{murray_consistent_2020}. Additionally, low
...
\citep{melotto_stomatal_2017}. A theoretical, optimized plant would
minimize stomatal density while also allowing competitive gas exchange
rates for its environment so as to maximize C assimilation per unit
investment in stomata. Natural selection operates within developmental
and physical constraints to drive each plant species toward its
theoretical optimum. This study provides evidence that stomata in
\emph{Arabidopsis thaliana}, the model angiosperm, are non-randomly
distributed, favoring dispersion over clustering
(\autoref{fig:single-surface}). However, stomata are not ideally
dispersed in an equilateral triangular grid as would be optimal to
minimize CO\(_2\) diffusion path length and standardize the area
...
across many plant lineages \citep{muir_light_2018}, yet much of the
dynamics of amphistomy remain poorly understood. Here, we show that in
\emph{Arabidopsis thaliana} 1) stomata in are non-randomly dispersed,
but not ideally dispersed; 2) stomatal size and density are modulated by
light; 3) stomatal size is positively correlated with the area to which
it supplies CO\(_2\); and 4) abaxial-adaxial stomatal coordination is
not exhibited and is not shown to provide a strong photosynthetic
advantage using CO\(_2\) diffusion models. Interestingly, these findings
did not validate many of our hypotheses which were based on first
principles, suggesting that there may be limits on plants' ability to
control stomatal placement. Future studies which elucidate these
limitations may have important implications for agriculturaly
